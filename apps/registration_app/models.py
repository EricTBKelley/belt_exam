...
class UserManager(models.Manager):
    def loginVal(self,postData):
        ...
        if len(email_matches) == 0:
            results['errors'].append('Please check to see if your email is correct.')
        else:
            results['user'] = email_matches[0]
            if not bcrypt.checkpw(postData['password'].encode(), results['user'].password.encode()):
                results['errors'].append('Please check to see if your password is correct.')
        return results
    ...
    def registerVal(self,postData):
        ...
        if not len(postData['birthday']):
            results['errors'].append('Please add a birthday')
    ...
    def createUser(self, postData):
        ...
        self.create(f_name=postData['f_name'], l_name=postData['l_name'], email=postData['email'], birthday = postData['birthday'], password= password)
